...
export class Device_interface extends Component {
	...
	onclick() {
		this.item.link.models[0].connections[0].connectedModelPointer.link.run = !this
			.item.link.models[0].connections[0].connectedModelPointer.link.run

		if (
			this.item.link.models[0].connections[0].connectedModelPointer.link.run
		) {
	...
	onclick2() {
		let speed = this.item.link.models[0].connections[0].connectedItemPointer
			.link.speed
	...
	render() {
		return (
			<div>
				...
				<button onClick={this.onclick.bind(this.item.a)}>
					{this.item.a.stopButton}
				</button>
				...
				<div class="slidecontainer">
					<input
						...
						min="0"
						max="10000"
						defaultValue={this.item.load}
						class="slider"
						id="myRange"
						onChange={this.handleChange.bind(this.item.a)}
						ref={this.item.a.value}
					/>
				</div>
			</div>
		)
