...
export default class PipeModel extends BaseModel {
	constructor(setup, parent, combine) {
		...
		if (combine) {
			...
			this.models.forEach(model => {
				...
				let index = model.parent.link.models.findIndex(m => {
					return m === model
				})
			...
			if (this.count === 1) {
				this.connections[1] = this.models[
	...
	reset() {
		...
		this.models.forEach(model => {
			...
			let index = model.parent.link.models.findIndex(model => {
				return model === this
			})
