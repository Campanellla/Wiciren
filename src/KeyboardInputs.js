		...
		if (this._onWheel){
			...
			if (this.wheel > 0.01 || (this.wheel < -0.01)){
				...
				if (camera.position.y < wmin) {
					...
					a = 3.14/3 + (camera.position.y - vmin) / (vmax - vmin) * 3.14/6;
					camera.rotation.x = a;
				}
				
				if (game.config.keyboardProgressiveSensibility){
					this.sensibility = game.config.keyboardSensibility + camera.position.y * game.config.keyboardProgressiveSensibilityMultiplier;
				}
